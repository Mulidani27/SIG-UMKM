...
class UmkmController extends Controller
{
    ...
    public function update(Request $request, Umkm $umkm)
    {
        ...
        if ($kecamatan && !$this->checkCoordinates($kecamatan->nama_kecamatan, $latitude, $longitude)) {
            return back()->withInput()->with('failed', 'Koordinat di luar kecamata');
        }
        if ($kelurahan && !$this->checkCoordinates($kelurahan->nama_kelurahan, $latitude, $longitude)) {
            return back()->withInput()->with('failed', 'Koordinat di luar kelurahan');
        }
    ...
    {
        $coordinates = [
            'Banjarmasin Utara' => [-3.2673603371098352, -3.3190513133673680, 114.5630938084325408, 114.6398500457430600],
            'Banjarmasin Tengah' => [-3.3039638433566552, -3.3328934462754769, 114.5711678848900874, 114.6069905844479990],
            'Banjarmasin Selatan' => [-3.3276707189285162, -3.3821400040578742, 114.5222812861525199, 114.6267710766239816],
            'Banjarmasin Barat' => [-3.2929059188730321, -3.3474377491527321, 114.5479693340175800, 114.5893565860376384],
            'Banjarmasin Timur' => [-3.3030794716402738, -3.3503296109227558, 114.5996158513904675, 114.6595890755754681],
            'Kota Banjarmasin' => [-3.2672272999999450, -3.3821391999999260, 114.5219479000001002, 114.6595898000000489],
        ];

        if (isset($coordinates[$nama_kecamatan])) {
            [$latMax, $latMin, $longMin, $longMax] = $coordinates[$nama_kecamatan];

            if ($latitude > $latMax || $latitude < $latMin || $longitude < $longMin || $longitude > $longMax) {
                return false;
            }
        }
        return true;
